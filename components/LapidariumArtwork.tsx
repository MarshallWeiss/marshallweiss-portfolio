...
export default function LapidariumArtwork({ compact = false, imageSrc }: LapidariumArtworkProps) {
  return (
    <div className={`lapidarium-art ${compact ? 'lapidarium-art--compact' : 'lapidarium-art--feature'}`} aria-hidden="true">
      <span className="lapidarium-wordmark">{compact ? 'L.' : 'Lapidarium'}</span>
      {!compact && (
        <>
          ...
          <span className="lapidarium-specimen-name">Amethyst<span>A study in form and light</span></span>
        </>
      )}
      <Image
        className="lapidarium-specimen"
        src={imageSrc || '/experiments/lapidarium/amethyst.webp'}
        width={800}
        height={800}
        alt=""
